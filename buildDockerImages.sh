...
if ! docker stats --no-stream >/dev/null 2>&1; then
	...
	TAG=${1:-$VERSION}
	DOCKER_PARAMS="--rm --name c0_patient_treatment_ui_build_docker_image --add-host=host.docker.internal:host-gateway -v /var/run/docker.sock:/var/run/docker.sock"
	if [[ "$OSTYPE" == "darwin"* ]]; then
		DOCKER_PARAMS="$DOCKER_PARAMS -e TESTCONTAINERS_HOST_OVERRIDE=docker.for.mac.host.internal"
	fi
	docker run $DOCKER_PARAMS -v "${HOME}/.m2":/root/.m2  -v "${PWD}":/app valawai/c0_patient_treatment_ui:dev ./mvnw clean package -DskipTests -Dquarkus.container-image.tag=$TAG
	if [ $? -ne 0 ]; then
