...
import eu.valawai.c0_patient_treatment_ui.messages.TreatmentValueFeedbackPayloadTest;
...
/**
 * Test the {@link TreatmentValueFeedbackEntity}.
 *
 * @author UDT-IA, IIIA-CSIC
 */
@QuarkusTest
@QuarkusTestResource(PostgreSQLTestResource.class)
public class TreatmentValueFeedbackEntityTest {
	...
	public void shouldNotStoreWithBadIdentifier(TransactionalUniAsserter asserter) {
		...
		asserter.assertFailedWith(() -> {
			...
			return TreatmentValueFeedbackEntity.store(payload);
	...
	public void shouldPersist(TransactionalUniAsserter asserter) {
		...
		asserter.assertThat(() -> {
			...
			final TreatmentValueFeedbackEntity stored = (TreatmentValueFeedbackEntity) asserter.getData("PERSISTED");
			var found = false;
			for (final var feedback : updated.valueFeedbacks) {

				if (feedback.id == stored.id) {
					found = true;
					final TreatmentValueFeedbackPayload payload = (TreatmentValueFeedbackPayload) asserter
							.getData("PAYLOAD");
					final long now = (Long) asserter.getData("NOW");
					assertTrue("Unexpected reported feedback time", now <= feedback.createdTime);
					assertEquals(payload.value_name, feedback.valueName);
